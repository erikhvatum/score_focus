...
def overallBackgroundSubtract(data_dir, match_string, temporal_radius, save_dir, save_dir2 = '', save_dir3 = '', demonstration_mode = False):
	'''
	...
	context_files = [freeimage.read(data_dir + '\\' + my_files[j]) for j in range(0, temporal_radius)]
	for i in range(temporal_radius, len(my_files)):
		real_raw_file = freeimage.read(data_dir + '\\' + my_files[i])
		...
		if demonstration_mode:
			freeimage.write(real_raw_file, save_dir + '\\' + my_files[i])
			freeimage.write(background_file, save_dir2 + '\\' + my_files[i])
			freeimage.write(final_mask, save_dir3 + '\\' + my_files[i])
